...
class WordGame:
    ...
    def show_continue_popup(self):
        ...
        popup.title("Next Step")
        popup.geometry("300x150")
        popup.configure(bg="#f9f9f9")

        tk.Label(popup, text="Do you want to continue?", font=('Helvetica', 14), bg="#f9f9f9").pack(pady=20)

        button_frame = tk.Frame(popup, bg="#f9f9f9")
        button_frame.pack()

        tk.Button(
            button_frame, text="Yes", bg="#4CAF50", fg="white", width=10,
            command=lambda: [popup.destroy(), self.continue_guessing()]
        ).pack(side="left", padx=10)

        tk.Button(
            button_frame, text="Exit", bg="#f44336", fg="white", width=10,
            command=lambda: [popup.destroy(), self.end_game(True)]
        ).pack(side="right", padx=10)

    def continue_guessing(self):
    # Enable the Done button so user can finish when ready
        if hasattr(self, "done_btn"):
            self.done_btn.config(state="normal")
        self.feedback_label.config(text="Keep going! Press 'Done' when finished.", fg="blue")
    ...
    def create_game_screen(self):
        ...
        self.update_timer()
        self.done_btn = tk.Button(self.root, text="Done", font=('Helvetica', 14), bg="#90ee90", command=lambda: self.end_game(True))
    ...
    def submit_word(self):
        ...
        if word in self.tracker.get_words():
            ...
            if word == bonus_word:
                ...
                self.update_score_label()  # <-- update here
                self.update_labels()
                if len(self.tracker.get_words()) == 5:
                    self.tracker.congrats_shown = True
                    self.feedback_label.config(text="🎉 Congratulations! You've guessed 5 correct words! 🎉", fg="blue")
                    self.update_labels()
                    self.end_game(True)
    ...
    def end_game(self, won):
        ...
        tk.Label(self.root, text="Suggest a word you think should be accepted, along with the level:", font=('Helvetica', 12)).pack(pady=(10, 2))
    ...
    def show_levels_screen(self):
        # Ensure name is set from entry before showing levels
        ...
        for i, (rel_x, rel_y) in zip(range(1, 6), button_rel_positions):
            btn = tk.Button(
                ...
                command=lambda level=i: (
                    ...
                    else messagebox.showinfo("Locked", "Slow down champ! Clear other levels!")
                )
            )
